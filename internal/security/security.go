...
func IsSymlinkToTLSFile(root, relPath string) bool {
	if relPath == "" {
		return false
	}
	fullPath := filepath.Join(root, filepath.FromSlash(relPath))
	...
	return IsLikelyTLSFile(resolved)
}
...
type TLSInodeIndex map[inodeKey]struct{}

func inodeKeyFromInfo(info os.FileInfo) (inodeKey, bool) {
...
func BuildTLSInodeIndex(root string) (TLSInodeIndex, error) {
	index := make(TLSInodeIndex)
	if root == "" {
		return index, nil
	}
...
func IsHardlinkToTLSFile(root, relPath string, tlsInodes TLSInodeIndex) bool {
	if relPath == "" || root == "" || len(tlsInodes) == 0 {
		return false
	}
	fullPath := filepath.Join(root, filepath.FromSlash(relPath))
	info, err := os.Lstat(fullPath)
...
func IsTLSFilePath(root, relPath string, tlsInodes TLSInodeIndex) bool {
	if IsLikelyTLSFile(relPath) {
		return true
	}
	if IsSymlinkToTLSFile(root, relPath) {
		return true
	}
	return IsHardlinkToTLSFile(root, relPath, tlsInodes)
}
